...
test_dataset = prepro.CustomDataset(root, train=True)
...
with torch.no_grad():
    ...
    for image, label in test_loader:
        ...
        output = net.encoder(image)
        output = np.reshape(output, (5120))
        print(output)
        print(label)
        np.save('en-output/output{}'.format(count), output)
        np.save('en-output/label{}'.format(count), label)
        count += 1